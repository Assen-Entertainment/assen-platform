...
stage_rename() {
    local from="$1"
    local to="$2"
    local from_path="${tmpdir}/${from}"
    mkdir -p "$(dirname "${from_path}")"
    if [ ! -f "${from_path}" ]; then
        printf "original\n" > "${from_path}"
        git -C "${tmpdir}" add "${from}"
        git -C "${tmpdir}" commit -q -m "add ${from}"
    fi
    mkdir -p "$(dirname "${tmpdir}/${to}")"
    git -C "${tmpdir}" mv "${from}" "${to}"
}
...
# ---------------------------------------------------------------------------
# 케이스 3: #31 기존 테스트 파일 이름 변경 차단
# ---------------------------------------------------------------------------
printf "\n[케이스 3] #31 기존 테스트 파일 이름 변경 차단\n"
setup_repo
stage_rename "server/tests/test_auth.py" "server/tests/auth_legacy.py"
assert_blocked "#31 test_*.py 이름 변경 R 차단"
teardown_repo

# ---------------------------------------------------------------------------
# 케이스 4: #31 *_test.py 수정 차단
# ---------------------------------------------------------------------------
printf "\n[케이스 4] #31 *_test.py 수정 차단\n"
setup_repo
...
# ---------------------------------------------------------------------------
# 케이스 5: ALLOW_TEST_EDIT=1 기존 테스트 수정 통과
# ---------------------------------------------------------------------------
printf "\n[케이스 5] ALLOW_TEST_EDIT=1 기존 테스트 수정 통과\n"
setup_repo
...
# ---------------------------------------------------------------------------
# 케이스 8: ALLOW_MIGRATIONS=1 마이그레이션 통과
# ---------------------------------------------------------------------------
printf "\n[케이스 8] ALLOW_MIGRATIONS=1 마이그레이션 통과\n"
setup_repo
stage_file "server/core/migrations/0001_initial.py" "# migration"
...
# ---------------------------------------------------------------------------
# 케이스 9: #31 골든 파일 커밋 차단 (/goldens/ 경로)
# ---------------------------------------------------------------------------
printf "\n[케이스 9] #31 골든 파일 커밋 차단\n"
setup_repo
...
# ---------------------------------------------------------------------------
# 케이스 10: ALLOW_GOLDEN=1 골든 파일 통과
# ---------------------------------------------------------------------------
printf "\n[케이스 10] ALLOW_GOLDEN=1 골든 파일 통과\n"
setup_repo
stage_file "apps/mobile/test/goldens/login_screen.png" "binary"
...
# ---------------------------------------------------------------------------
# 케이스 12: #27 .env.production 커밋 차단
# ---------------------------------------------------------------------------
printf "\n[케이스 12] #27 .env.production 차단\n"
setup_repo
...
# ---------------------------------------------------------------------------
# 케이스 16: #27 .env.sample 허용 / #27 .env.template 허용
# ---------------------------------------------------------------------------
printf "\n[케이스 16] #27 .env.sample 허용\n"
setup_repo
