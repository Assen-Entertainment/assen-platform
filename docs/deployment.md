...
> **2026-07-09 마이그레이션 정식 전환 완료**라 각 앱이 `0001_initial`을 보유하며
> **`migrate`가 테이블 생성 경로**(up-local에 배선, prod는 one-off task 심;
> 모델 변경 시 `makemigrations`, server/AGENTS.md), 프로덕션 설정은
> `config.settings.prod`(fail-closed).
