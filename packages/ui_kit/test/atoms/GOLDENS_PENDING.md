# Atom golden baselines — pending human approval (#31)

Every atom test declares a pixel `goldenTest(...)` with **`skip: true`**. The
...
The interactive/render/variant behaviour of every atom is already covered by
non-skipped `testWidgets` assertions in the sibling `*_test.dart` files, so the
atoms are verified independently of the pixel baselines.
