...
Requires ``DJANGO_SETTINGS_MODULE`` to be set explicitly — there is no dev
fallback (ASS-285): a deployment that omits it fails loudly rather than silently
booting dev (DEBUG + every mock gate on). Django is
